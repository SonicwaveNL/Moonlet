...
from interpreter.program import Program, Scope
from interpreter.errors import Error, FileNotFoundError
...
class Launcher:
    ...
    def __init__(
        self, file_path: Optional[str] = None, debug_mode: bool = False, test_mode=False
    ) -> None:
        ...
        if test_mode:
            print(f"{'TESTING':-^60}", end="\n\n")
            print(f"{'UNIT TESTS':-^60}")
            subprocess.check_call(["python3", "-m", "unittest", "tests.unit_tests"])
            print(f"{'INTERGRATION TEST':-^60}")
            subprocess.check_call(
                ["python3", "-m", "unittest", "tests.intergration_test"]
            )
            print(f"{'SYSTEM TEST':-^60}")
            subprocess.check_call(["python3", "-m", "unittest", "tests.system_test"])
            print(f"{'='*60}")

        elif file_path is not None and len(file_path) > 0:
            ...
            # Also check if the 'file_path' has the Moonlet extention
                ...
                print(
                    FileNotFoundError(
                        f"File '{file_path}' has an invalid extention (must be '.mnl')"
                    )
                )
    ...
    def run_moonlet(self):
        ...
        with open(self.file_path, "r") as file:
            ...
            if lexer_error is not None:
                return self.print_error(lexer_error)
            ...
            if ats.error is not None:
                return self.print_error(ats.error)
            ...
            if prog_result.error is not None:
                return self.print_error(prog_result.error)
